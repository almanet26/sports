...
export default function CoachPlayerPerformance() {
  ...
  const summaryCards = [
    ...
    {
      label: 'Last Active',
      value: summary.days_since_last_submission === 0 ? 'Today'
        : summary.days_since_last_submission === 1 ? 'Yesterday'
        : summary.days_since_last_submission !== null ? `${summary.days_since_last_submission}d ago`
        : 'N/A',
      icon: 'fas fa-clock',
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              ...
              {flaw_trend && (
                <span className={`px-3 py-1 rounded-full text-xs border ${
                  flaw_trend.delta < 0 ? 'bg-green-500/20 text-green-400 border-green-500/30'
                  : flaw_trend.delta > 0 ? 'bg-red-500/20 text-red-400 border-red-500/30'
                  : 'bg-white/10 text-white/50 border-white/20'
                }`}>
                  {flaw_trend.delta < 0 ? `${Math.abs(flaw_trend.delta)} fewer flaws`
                    : flaw_trend.delta > 0 ? `${flaw_trend.delta} more flaws`
                    : 'No change in flaws'}
                </span>
              )}
            </div>
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={typeData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: 'rgba(0,0,0,0.85)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', color: 'white' }} />
                <Bar dataKey="count" radius={[6, 6, 0, 0]} fill="url(#typeGrad)" />
                <defs>
          ...
          {flaw_trend ? (
            <div className="space-y-4">
              ...
              <div className={`flex items-center justify-between p-4 rounded-xl border ${
                flaw_trend.delta < 0 ? 'bg-green-500/10 border-green-500/30'
                : flaw_trend.delta > 0 ? 'bg-red-500/10 border-red-500/30'
              ...
              {submission_timeline.map((sub, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${sub.analysis_type === 'BATTING' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>
                      {sub.analysis_type}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {sub.pdf_report_url ? (
                      <a href={resolveMediaUrl(sub.pdf_report_url)} target="_blank" rel="noopener noreferrer"
