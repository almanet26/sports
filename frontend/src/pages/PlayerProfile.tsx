...
export default function PlayerProfile() {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const trimmedProfile: PlayerProfileState = {
        ...draftProfile,
      ...
      const completionData = getProfileCompletion(trimmedProfile);
      const nextProfile = { ...trimmedProfile, ...completionData };

      persistLocalProfile(user, nextProfile);
      ...
      setIsEditOpen(false);
      setPageMessageType("success");
      setPageMessage("Profile updated locally for UI preview.");

      if (user) {
        const nextUser = { ...user, name: nextProfile.fullName || user.name, profile_bio: nextProfile.bio };
  ...
  const handleVideoSelection = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const localVideo: PlayerVideoItem = {
        id: `${Date.now()}`,
      ...
      setUploadedVideos((current) => [localVideo, ...current]);
      setPageMessageType("success");
      setPageMessage("Video added locally for UI preview.");
    } catch (error) {
      setPageMessageType("error");
  ...
  const handleDeleteVideo = async (videoId: string) => {
    ...
    try {
      ...
      setPageMessageType("success");
      setPageMessage("Video removed from local UI preview.");
    } catch (error) {
      setPageMessageType("error");
