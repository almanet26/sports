...
import { earningsApi, type EarningsData } from '../lib/api';
...
export default function CoachEarningsPage() {
  ...
  const stats = [
    { label: 'Total Earned', value: `$${data?.total_earned ?? 0}`, icon: 'fas fa-dollar-sign', color: 'from-green-500 to-emerald-500' },
    { label: 'Pending', value: `$${data?.pending ?? 0}`, icon: 'fas fa-clock', color: 'from-yellow-500 to-orange-500' },
    { label: 'This Month', value: `$${data?.this_month ?? 0}`, icon: 'fas fa-calendar', color: 'from-blue-500 to-cyan-500' },
  ];
  ...
  return (
    <div className={theme === 'dark' ? 'text-white' : 'text-gray-900'}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        ...
        {loading ? (
          <p className={`p-6 text-sm ${sub}`}>Loading...</p>
        ) : !data?.transactions.length ? (
          <p className={`p-6 text-sm ${sub}`}>No transactions yet.</p>
        ) : (
              ...
              <motion.div key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${sub}`}>{t.type} · {t.date ? new Date(t.date).toLocaleDateString([], { month: 'short', day: 'numeric' }) : ''}</p>
                  </div>
                </div>
