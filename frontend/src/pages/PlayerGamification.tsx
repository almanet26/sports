    ...
    <div className="text-white space-y-8">
      ...
      <div className="grid sm:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          className="glass rounded-2xl p-6 border border-white/20 flex flex-col items-center text-center gap-2">
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                  <motion.div
                    ...
                    style={{ originY: 1 }}
                    className={`w-full rounded-lg ${day.active ? 'bg-gradient-to-t from-orange-500 to-amber-400 h-10' : 'bg-white/8 h-5'}`}
                  />
                  <p className={`text-[10px] font-medium ${day.active ? 'text-orange-400' : 'text-white/25'}`}>{label}</p>
                </div>
              );
            })}
          </div>
      ...
      <AnimatePresence>
        {badges.next_badge && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: 0.2 }}
            className="glass rounded-2xl p-5 border border-blue-500/25 bg-gradient-to-r from-blue-500/5 to-purple-500/5">
              ...
              <div className="flex-1 min-w-0 space-y-2">
                ...
                <div className="h-2 rounded-full bg-white/8 overflow-hidden">
                  <motion.div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-purple-500"
                    initial={{ width: 0 }} animate={{ width: `${badges.next_badge.progress_pct}%` }}
                    transition={{ duration: 1, delay: 0.3 }} />
                </div>
                <p className="text-xs text-white/35">{badges.next_badge.progress_current} / {badges.next_badge.progress_target}</p>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <AnimatePresence mode="wait">
          {filtered.length > 0 ? (
            <motion.div key={filter} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              {filtered.map((b, i) => <BadgeCard key={b.key} badge={b} index={i} />)}
            </motion.div>
          ) : (
