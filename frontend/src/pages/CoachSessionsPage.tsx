...
import { sessionsApi, type TrainingSession } from '../lib/api';
...
export default function CoachSessionsPage() {
  ...
  useEffect(() => {
    sessionsApi.list().then(r => setSessions(r.data.sessions)).catch(() => setSessions([])).finally(() => setLoading(false));
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        const r = await sessionsApi.update(editing.id, form);
        setSessions(prev => prev.map(s => s.id === editing.id ? r.data : s));
      } else {
        const r = await sessionsApi.create(form);
