...
export default function CoachTrainingPlansPage() {
  ...
  const addDrill = () => { if (!drillInput.trim()) return; setForm(f => ({ ...f, drills: [...(f.drills || []), drillInput.trim()] })); setDrillInput(''); };
  const removeDrill = (i: number) => setForm(f => ({ ...f, drills: (f.drills || []).filter((_, idx) => idx !== i) }));
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="space-y-3">
                <div><label className={`block text-xs mb-1 ${sub}`}>Title *</label><input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className={inputCls} /></div>
                <div><label className={`block text-xs mb-1 ${sub}`}>Description</label><textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} className={inputCls + ' resize-none'} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={`block text-xs mb-1 ${sub}`}>Type</label>
                    <select value={form.analysis_type} onChange={e => setForm(f => ({ ...f, analysis_type: e.target.value }))} className={inputCls}>
                  ...
                  <div><label className={`block text-xs mb-1 ${sub}`}>Plan For</label>
                    <select value={form.plan_type} onChange={e => setForm(f => ({ ...f, plan_type: e.target.value as TrainingPlanCreate['plan_type'] }))} className={inputCls}>
