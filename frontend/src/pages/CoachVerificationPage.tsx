...
import { adminCoachesApi, type AdminCoachRecord } from '../lib/api';
...
export default function CoachVerificationPage() {
  ...
  const loadPendingCoaches = async () => {
    try {
      const res = await adminCoachesApi.pending();
  ...
  const handleApprove = async (coachId: string) => {
    try {
      await adminCoachesApi.approve(coachId);
  ...
  const handleReject = async (coachId: string) => {
    ...
    try {
      await adminCoachesApi.reject(coachId);
  ...
  const downloadDocument = async (coachId: string, coachName: string) => {
    try {
      const response = await adminCoachesApi.downloadDocument(coachId);
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-sm text-white/60">
                    ...
                    <p>
                      ...
                      Applied: {coach.created_at ? new Date(coach.created_at).toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
