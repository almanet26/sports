...
# New features from test branch
try:
    from api.routes.earnings import router as earnings_router
    app.include_router(earnings_router, prefix="/api/v1", tags=["earnings"])
except Exception as e:
    logger.warning("Earnings routes disabled: %s", e)
...
try:
    ...
    logger.warning("Sessions routes disabled: %s", e)
...
try:
    ...
    logger.warning("Coach content routes disabled: %s", e)

try:
    from api.routes.messages import router as messages_router
    app.include_router(messages_router, prefix="/api/v1", tags=["messages"])
except Exception as e:
    logger.warning("Messages routes disabled: %s", e)
