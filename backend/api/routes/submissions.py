...
def coach_player_progress(
    ...
    return {
        "player": {"id": player.id, "name": player.name, "email": player.email, "team": player.team},
        "summary": {
...
def coach_accept_submission(
    ...
    if sub.status != SubmissionStatus.PENDING:
        raise HTTPException(status_code=409, detail=f"Submission is already {sub.status.value}.")
