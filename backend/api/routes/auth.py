...
async def register(
    ...
    if role == 'COACH' and not document_type:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Document type is required for coach registration"
        )

    if role == 'COACH' and not coach_document_url:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Verification document upload is required for coach registration"
        )
