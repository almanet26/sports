...
class GeminiManager:
    def __init__(self):
        ...
        def _maybe_add(val: str | None) -> None:
            ...
            if (
                ...
                or len(key) < 20
                or not re.fullmatch(r"(AIza[A-Za-z0-9_-]{20,}|AQ\.[A-Za-z0-9_\-+/=]{20,})", key)
            ):
                rejected += 1
                return
            if key in seen:
        ...
        if self.keys:
            logger.info("[GeminiManager-bowling] %d key(s) loaded, model=%s", len(self.keys), self.model_name)
        else:
            logger.warning(
                "[GeminiManager-bowling] No valid Gemini keys found "
                "(checked GEMINI_API_KEY_1..5, GEMINI_API_KEY, GOOGLE_GEMINI_API_KEY, GOOGLE_API_KEY). "
                "rejected_values=%d — AI feedback will be skipped.",
                rejected,
            )
    ...
    def _fallback_feedback(self, reason: str = "not_configured") -> str:
        if reason == "not_configured":
            msg = (
                "AI feedback is temporarily unavailable because no Gemini API key is configured "
                "for this environment (GEMINI_API_KEY_1 is missing or invalid in Secret Manager). "
                "The biomechanics analysis and annotated video were still generated by the local pipeline, "
                "so review those results and edit this draft manually before publishing."
            )
        else:
            msg = (
                f"AI feedback is temporarily unavailable (Gemini API error: {reason}). "
                "The biomechanics analysis and annotated video were still generated by the local pipeline, "
                "so review those results and edit this draft manually before publishing."
            )
        return msg
    ...
    def call_gemini(self, prompt, video_path=None, _retry_count: int = 0):
        ...
        if not client:
            return self._fallback_feedback("not_configured")
        ...
        try:
            ...
            if video_path:
                try:
                    ...
                    logger.warning("Gemini video upload failed for bowling analysis: %s", e)
                    return self._fallback_feedback(f"video upload error: {type(e).__name__}")
            ...
            if is_retryable and _retry_count < MAX_RETRIES:
                wait = 2 ** (_retry_count + 1)
                logger.warning("Gemini rate-limited, retrying in %ds (attempt %d/%d)", wait, _retry_count + 1, MAX_RETRIES)
                time.sleep(wait)
                self.current_index += 1
                return self.call_gemini(prompt, video_path, _retry_count + 1)
            reason = "invalid_api_key" if is_auth_invalid else ("rate_limited" if is_retryable else type(e).__name__)
            logger.warning("Gemini fallback triggered for bowling analysis — reason=%s: %s", reason, e)
            return self._fallback_feedback(reason)
