...
class BattingGeminiManager:
    ...
    def _fallback_feedback(self) -> str:
        return (
            "AI feedback is temporarily unavailable because Gemini is not configured for this environment. "
            "The biomechanics analysis and annotated video were still generated by the local pipeline, so review "
            "those results and edit this draft manually before publishing."
        )
    ...
    def call_gemini(self, prompt: str, video_path: str | None = None, _retry_count: int = 0) -> str:
        ...
        try:
            ...
            if is_retryable and _retry_count < MAX_RETRIES:
                ...
                return self.call_gemini(prompt, video_path, _retry_count + 1)
            logger.warning("Gemini fallback triggered for batting analysis: %s", e)
            return self._fallback_feedback()
