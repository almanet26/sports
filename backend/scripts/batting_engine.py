...
class BattingGeminiManager:
    ...
    def __init__(self) -> None:
        ...
        if self.keys:
            logger.info("[BattingGeminiManager] %d key(s) loaded, model=%s", len(self.keys), self.model_name)
        else:
            logger.warning(
                "[BattingGeminiManager] No valid Gemini keys found "
        ...
        try:
            ...
            if video_path:
                try:
                    ...
                    logger.warning("Gemini video upload failed for batting analysis: %s", e)
            ...
            logger.warning("Gemini fallback triggered for batting analysis — reason=%s: %s", reason, e)
